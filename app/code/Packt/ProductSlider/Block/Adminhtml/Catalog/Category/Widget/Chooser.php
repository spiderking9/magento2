...
class Chooser extends \Magento\Catalog\Block\Adminhtml\Category\Widget\Chooser {
    ...
    public function prepareElementHtml(\Magento\Framework\Data\Form\Element\AbstractElement $element){
        ...
        $chooser = $this->getLayout()->createBlock('Magento\Widget\Block\Adminhtml\Widget\Chooser')->setElement($element)->setConfig($this->getConfig()->setFieldsetId())->setSourceUrl($sourceUrl)->setUniqId($uniqId);
    ...
    public function getNodeClickListner() {
        ...
        if($this->getUseMassaction()) {
            ...
            $js = '
                function (node,e) {
                    '.$chooserJsObject.'.setElementValue(node.attributes.id);'.$chooserJsObject.'.setElementLabel(node.text);'.$chooserJsObject.'.close();
                }';
        }
        return $js;
    }
