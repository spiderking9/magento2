...
class Filter extends FilterModel
{
	...
	public function getLayerConfiguration($filters, $config)
	{
		...
		$config->addData([
			'infiniteScroll' => (bool)$this->helper->getGeneralConfig('infinite_scroll'),
	...
	public function getDisplayType($filter)
	{
		if ($filter->hasAttributeModel()) {
			...
			if ($displayType == DisplayType::TYPE_SCROLL) {
				...
				$displayType = $this->helper->getGeneralConfig(LayerHelper::FIELD_DISPLAY_TYPE);
				if ($displayType == DisplayType::TYPE_SCROLL) {
					return [
						'type'  => 'scroll',
	...
	protected function getDisplayValue($type, $attribute = null)
	{
		...
		return max($minValue, $this->helper->getGeneralConfig($type) ?: $defaultValue);
	}
}