		...
		if (version_compare($context->getVersion(), '2.1.0', '<')) {
			if (!$installer->tableExists('layered_product_pages')) {
				$table = $installer->getConnection()->newTable($installer->getTable('layered_product_pages'))
					->addColumn('page_id', \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER, null, ['identity' => true, 'unsigned' => true, 'nullable' => false, 'primary' => true])
					->addColumn('name', \Magento\Framework\DB\Ddl\Table::TYPE_TEXT, '256', [])
					->addColumn('page_title', \Magento\Framework\DB\Ddl\Table::TYPE_TEXT, '256', [])
					->addColumn('status', \Magento\Framework\DB\Ddl\Table::TYPE_SMALLINT, 1, ['nullable' => false, 'default' => 1])
					->addColumn('route', \Magento\Framework\DB\Ddl\Table::TYPE_TEXT, '255', [])
					...
					->addColumn('default_attributes', \Magento\Framework\DB\Ddl\Table::TYPE_TEXT, '2M', [])
					->addColumn('meta_title', \Magento\Framework\DB\Ddl\Table::TYPE_TEXT, '256', [])
					->addColumn('meta_keywords', \Magento\Framework\DB\Ddl\Table::TYPE_TEXT, '256', [])
					->addColumn('meta_description', \Magento\Framework\DB\Ddl\Table::TYPE_TEXT, '2M', [])
					->addIndex($installer->getIdxName('layered_product_pages', 'route'), 'route', ['type' => \Magento\Framework\DB\Adapter\AdapterInterface::INDEX_TYPE_UNIQUE])
