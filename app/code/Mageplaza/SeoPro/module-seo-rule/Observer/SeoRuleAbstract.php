...
abstract class SeoRuleAbstract
{
    ...
    /**
     * @var \Mageplaza\SeoRule\Model\RuleFactory
     */
    protected $ruleFactory;

    /**
     * @var \Mageplaza\SeoRule\Model\MetaFactory
     */
        ...
        if ($type == Type::LAYERED_NAVIGATION) {
            ...
            $ruleCollection = $this->ruleFactory->create()
                ...
                ->addFieldToFilter('stores', [
                    ['finset' => \Magento\Store\Model\Store::DEFAULT_STORE_ID],
                    ['finset' => $currentStore]
                ])
            ...
            foreach ($this->request->getParams() as $key => $param) {
                ...
                if ($attKey = $productResource->getAttribute($key)) {
                    $dataAttributeFilter['{{' . $key . '}}'] = $attKey->getStoreLabel() . ' ' . $attKey->getSource()->getOptionText($param);
                }
            }
            $dataAttributeFilter['{{category_name}}'] = $object->getName();

            $metaModel->setMetaTitle($this->helperData->generateMetaTemplateForLayer($metaModel->getMetaTitle(), $dataAttributeFilter));
            $metaModel->setMetaDescription($this->helperData->generateMetaTemplateForLayer($metaModel->getMetaDescription(), $dataAttributeFilter));
            $metaModel->setMetaKeywords($this->helperData->generateMetaTemplateForLayer($metaModel->getMetaKeywords(), $dataAttributeFilter, true));
        } else {
            ...
            if ($type == Type::PRODUCTS) {
                $metaModel->setMetaTitle($this->helperData->replaceCategoryName($object, $metaModel->getMetaTitle(), $type));
                $metaModel->setMetaDescription($this->helperData->replaceCategoryName($object, $metaModel->getMetaDescription(), $type));
                $metaModel->setMetaKeywords($this->helperData->replaceCategoryName($object, $metaModel->getMetaKeywords(), $type));
            }
        }
