...
class Data extends AbstractHelper
{
    ...
    public function getSeoRuleConfig($code = '', $storeId = null)
    {
        ...
        return $this->getConfigValue(static::CONFIG_MODULE_PATH . '/' . self::SEO_RULE_CONFIGUARATION . $code, $storeId);
    }
    ...
    public function processOptions($template, $product, $type = 'product')
    {
        ...
        foreach ($match[1] as $key => $attribute) {
            if ($type == Type::PAGES) {
                ...
                } else if ($product->getResource()->getAttribute($attribute) && $product->getResource()->getAttribute($attribute)->usesSource()) {
                ...
                } else if (in_array($attribute, ['price', 'special_price'])) {
                    if (!empty($product->getData($attribute))) {
                        $productData[$match[0][$key]] = $this->priceHelper->currency($product->getData($attribute), true, false);
                    }
                } else {
                    if ($product->getData($attribute)) {
                        $productData[$match[0][$key]] = $product->getData($attribute);
     ...
     *
     * @param string $entity
     * @return \Magento\Catalog\Model\Category|\Magento\Catalog\Model\Product|\Magento\Cms\Model\Page
     */
    ...
    public function applyRuleId($model, $metaModel)
    {
        ...
        if (is_array($listId)) {
            ...
            foreach ($listId as $id) {
                $data['meta_title']       = $this->generateMetaTemplate($model->getMetaTitle(), $id, false, $model->getEntityType());
                $data['meta_description'] = $this->generateMetaTemplate($model->getMetaDescription(), $id, false, $model->getEntityType());
                $data['meta_keywords']    = $this->generateMetaTemplate($model->getMetaKeywords(), $id, true, $model->getEntityType());
    ...
    public function replaceCategoryName($object, $meta, $type)
    {
        ...
        } else if (count($object->getCategoryIds())) {
