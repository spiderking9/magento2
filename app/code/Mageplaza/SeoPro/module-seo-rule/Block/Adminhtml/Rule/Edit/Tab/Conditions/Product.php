...
class Product extends Generic implements TabInterface
{
     ...
     */
    protected function addTabToForm($model, $fieldsetId = 'conditions_fieldset', $formName = 'mageplaza_seorule_rule_form')
    {
        ...
        if ($model->getBlockType() == 'product') {
            $renderer->setAjaxUrl($this->getUrl('seo/grid/productlist', ['id' => $id, 'type' => 'cond', 'form_key' => $this->formKey->getFormKey()]));
        }
        ...
        if ($seoRuleType == \Mageplaza\SeoRule\Model\Rule\Source\Type::LAYERED_NAVIGATION) {
